...
function AddDireccionCard({ onClose, onGuardar }: AddDireccionCardProps) {
    ...
    useEffect(() => {
        fetch('https://tp-dsw-backend-yjx3.onrender.com/api/localidades')
            ...
            .catch((err) => {
                console.error(err)
                setErrorLocalidades('Error al cargar las localidades');
            })
    ...
    return (
        <div className='card-add-direccion'>
            ...
            <form onSubmit={handleEnviar}>
                ...
                {cargandoLocalidades ? (
                    <p className='map-cargando'>Cargando localidades...</p>
                ) : errorLocalidades ? (
                    <p style={{ color: 'red', fontSize: '0.85rem' }}>{errorLocalidades}</p>
                ) : (
                    <select
                        value={localidadId}
